...
public class CoberturaPublisher extends Recorder {
	...
	public CoberturaPublisher(final String coberturaReportFile, final boolean onlyStable, final boolean failUnhealthy,
			final boolean failUnstable, final boolean autoUpdateHealth, final boolean autoUpdateStability,
			final SourceEncoding sourceEncoding) {
	 ...
	 */
	private void setTargets(final List<CoberturaPublisherTarget> targets) {
		healthyTarget.clear();
	 ...
	 */
	public void setHealthyTarget(final CoverageTarget healthyTarget) {
	 ...
	 */
	public void setUnhealthyTarget(final CoverageTarget unhealthyTarget) {
	 ...
	 */
	public void setFailingTarget(final CoverageTarget failingTarget) {
	 ...
	 */
	/* package */
	static File getCoberturaReportDir(final AbstractItem project) {
	 ...
	 */
	/* package */
	static File[] getCoberturaReports(final AbstractBuild<?, ?> build) {
	...
	public boolean perform(final AbstractBuild<?, ?> build, final Launcher launcher, final BuildListener listener)
		...
		if (build.getResult().isWorseThan(threshold)) {
			listener.getLogger().println(
					"Skipping Cobertura coverage report as build was not " + threshold.toString() + " or better ...");
			return true;
		}
		...
		if (reports.length == 0) {
			String msg = "No coverage results were found using the pattern '" + coberturaReportFile + "' relative to '"
					+ moduleRoot.getRemote() + "'." + "  Did you enter a pattern relative to the correct directory?"
		...
		for (int i = 0; i < reports.length; i++) {
			...
			try {
				...
				Util.displayIOException(e, listener);
				e.printStackTrace(listener.fatalError("Unable to copy coverage from " + reports[i] + " to "
						+ buildTarget));
				build.setResult(Result.FAILURE);
			}
		}
		...
		if (result != null) {
			...
			final CoberturaBuildAction action = CoberturaBuildAction.load(build, result, healthyTarget,
					unhealthyTarget, getOnlyStable(), getFailUnhealthy(), getFailUnstable(), getAutoUpdateHealth(),
	 ...
	 */
	private void setNewPercentages(final CoverageResult result, final boolean select, final BuildListener listener) {
			...
			for (CoverageMetric metric : healthyMetrics) {
				...
				if (newPercent > oldPercent) {
					if (select) {
						...
						listener.getLogger().println(
								"    " + metric.getName() + "'s new health minimum is: " + newPercent);
					} else {
						failingTarget.setTarget(metric, newPercent);
						listener.getLogger().println(
								"    " + metric.getName() + "'s new stability minimum is: " + newPercent);
	...
	public static final class DescriptorImpl extends BuildStepDescriptor<Publisher> {
		CoverageMetric[] metrics = { CoverageMetric.PACKAGES, CoverageMetric.FILES, CoverageMetric.CLASSES,
				CoverageMetric.METHOD, CoverageMetric.LINE, CoverageMetric.CONDITIONAL, };
		...
		public List<CoberturaPublisherTarget> getTargets(final CoberturaPublisher instance) {
			if (instance == null) {
		 ...
		 */
		@Override
		public CoberturaPublisher newInstance(final StaplerRequest req, final JSONObject formData) throws FormException {
			...
			List<CoberturaPublisherTarget> targets = req.bindParametersToList(CoberturaPublisherTarget.class,
					"cobertura.target.");
	...
	private boolean didCoberturaRun(final Iterable<MavenBuild> mavenBuilds) {
	...
	private boolean didCoberturaRun(final MavenBuild mavenBuild) {
		for (ExecutedMojo mojo : mavenBuild.getExecutedMojos()) {
			if (mojo.groupId.equals("org.codehaus.mojo")
					&& (mojo.artifactId.equals("cobertura-maven-plugin") || mojo.artifactId
							.equals("cobertura-it-maven-plugin"))) {
				return true;
			}
		}
		return false;
